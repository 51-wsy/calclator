;(function(global) {
  ...
  // 初始化
  function initEvent() {
    actionDiv.addEventListener('click', handleClickEvent, false); // 冒泡获取
  ...
  function setCalcLine(type) {
    ...
    if(text) {
      calcStack.pop()
      calcStack.push(text)
      console.log(calcStack,lastNum,lastOp);
    }*/
  ...
  function formatNumber(number) {
    ...
    if (decimal.toString().indexOf('e') !== -1) {
      ...
      // return decimal.toString();
      return decimal.toExponential(15);
    } else {
      const result = decimal.toDecimalPlaces(15); //最多保留10/15位小数
  ...
  function calcFinal(type) {  
	...
	// 判断当前输入是否仅为数字
  //let isNumber = current.length === 1 && !isNaN(Number(current[0]));
// 检查是否需要进行连续等号运算
  /*isContinued = type === 'equal' && isNumber;
  // 判断是否需要进行连续等号的运算
  if (isContinued && isNumber) {
    current.unshift(lastNum);
    current.unshift(lastOp);
  }*/
