;(function(global) {
  ...
  function computedCalc(text,type) {
    ...
    if (Object.keys(symbol).includes(calcLineText[calcLineText.length - 1]) && !current.length) {
      ...
      current = [];
      let pushData = [];
      let str = removeDecimal(_current.join(''));
      _current = [str];
  ...
  function formatNumber(number) {
    ...
    } else if (decimal.abs().greaterThan('9999999999999999')) {
      // 如果绝对值大于1000000，则采用科学计数法表示，保留6位小数
      // return decimal.toString();
      return decimal.toExponential(10);
    } else {
      const result = decimal.toDecimalPlaces(10); //最多保留10位小数
      return result.toFixed(result.decimalPlaces());
    }
  }
  // 计算表达式最后的值
  function calcFinal(type) {
    // 如果已经运算过，则是四则运算的连等运算事件或特殊运算
    if (isEqualed) {
      // 四则运算的连等运算事件
